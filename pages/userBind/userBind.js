...
// 发送验证码的方法
  wx.request({
    url: 'https://api.joinsilk.com/Api?appCode=silkPlatform&token=&url=/pfuser/sendVerCode',
    method: 'get',
    data: {
  ...
  // },
  yzmBlur:function(e){   //请输入验证码输入完之后，光标离开
    var yzCO = e.detail.value;
    var that = this;
    console.log(yzCO)
    if (yzCO.split('').length == 4){
      var reg = /^1[34578]\d{9}$/;  //验证手机号码正则表达式
      ...
      console.log(val + 'ssssssssssss' + yzCO);
      console.log(yzCO);
      if ((yzCO != '') && (reg.test(val))) {
  ...
  setLoading:function(){
    var that = this;
    var wxAccount = '';
    if (app.globalData.wxAccount == '' || app.globalData.wxAccount == null || app.globalData.wxAccount==undefined){
      wxAccount = app.globalData.wxAccount
    }else{
      wxAccount = wx.getStorageSync('wxAccount')
    }
    ...
    if (reg.test(that.data.inputVal) && (that.data.yzmCode != '') && (that.data.inputVal!='')){
      ...
      wx.request({
        url: 'https://api.joinsilk.com/Api?appCode=silkPlatform&token=&url=/pfuser/bindWxAndPhone',
        method: 'get',
        data: {
